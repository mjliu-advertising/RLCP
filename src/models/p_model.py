...
# Traditional click-through rate prediction model
class LR(nn.Module):
    def __init__(self,
                 feature_nums,
                 output_dim=1):
...
# Click-through rate prediction model based on Deep learning
...
class DeepFM(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bias + torch.sum(self.linear(linear_x), dim=1) + ix * 0.5 + self.mlp(
