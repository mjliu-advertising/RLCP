...
def get_dataset(args):
    ...
    field_nums = train_data.shape[1] - 1  # 17
...
if __name__ == '__main__':
    ...
    #
    # train_ctrs = ctr_model(torch.LongTensor(train_data[:, 5:]).to(args.device)).detach().cpu().numpy()  # 6-12
    #
    # test_ctrs = ctr_model(torch.LongTensor(test_data[:, 5:]).to(args.device)).detach().cpu().numpy()  # 13-15
    #
    # # click + winning price + hour + timestamp + encode 生成FAB要用的数据
    # train_data = {'clk': train_data[:, 0].tolist(),
    #               'ctr': train_ctrs.flatten().tolist(),
    #               'mprice': train_data[:, 1].tolist(),
    #               'hour': train_data[:, 2].tolist(),
    #               'time_frac': train_data[:, 3].tolist(),
    #               'timestamp': train_data[:, 4].tolist()}
    #
    # test_data = {'clk': test_data[:, 0].tolist(),
    #              'ctr': test_ctrs.flatten().tolist(),
    #              'mprice': test_data[:, 1].tolist(),
    #              'hour': test_data[:, 2].tolist(),
    #              'time_frac': test_data[:, 3].tolist(),
    #              'timestamp': train_data[:, 4].tolist()}
    #
    # data_path = args.data_path + args.dataset_name + args.campaign_id
    # train_df = pd.DataFrame(data=train_data)
    # train_df.to_csv(data_path + 'train.bid.' + args.sample_type + '.fab.csv', index=None)
    #
    # test_df = pd.DataFrame(data=test_data)
    # test_df.to_csv(data_path + 'test.bid.' + args.sample_type + '.fab.csv', index=None)
