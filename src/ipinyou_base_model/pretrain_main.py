...
if __name__ == '__main__':
    campaign_id = '1458/' # 1458, 3358, 3386, 3427, 3476
    ...
    model_name = 'W&D'
    test_model = get_model(model_name, feature_nums, field_nums, args.latent_dims).to(device)
    load_path = args.save_param_dir + args.campaign_id + model_name + '_all_best.pth'
    test_model.load_state_dict(torch.load(load_path, map_location=device))  # 加载最优参数
    test_predicts, test_auc = submission(test_model, test_data_loader, device)
    val_predicts, val_auc = submission(test_model, val_data_loader, device)

    val_pred_df = pd.DataFrame(data=val_predicts)
    submission_path = args.data_path + args.dataset_name + args.campaign_id + model_name + '/'  # ctr 预测结果存放文件夹位置
    val_pred_df.to_csv(submission_path + 'val_submission_' + args.sample_type + '.csv', index=None)
    threads = []
    for model_name in choose_models:
        ...
        current_model_test_predicts, current_model_train_predicts, current_model_val_predicts = main(model, model_name, train_data_loader,
    ...
    for key in test_predict_arr_dicts.keys():
        ...
        # 训练集
        final_train_sub = np.mean(train_predict_arr_dicts[key], axis=0)
        train_pred_df = pd.DataFrame(data=final_train_sub)
        train_pred_df.to_csv(submission_path + 'train_submission_' + args.sample_type + '.csv', index=None)
        # 验证集
        final_val_sub = np.mean(val_predict_arr_dicts[key], axis=0)
        val_pred_df = pd.DataFrame(data=final_val_sub)
        val_pred_df.to_csv(submission_path + 'val_submission_' + args.sample_type + '.csv', index=None)
